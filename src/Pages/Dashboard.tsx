...
const Dashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch report status
      const reportResponse = await fetch('http://localhost:3000/api/auth/report-status', { headers });
  ...
  const handleDownload = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3000/api/files/download/${reportPath}`, {
        method: 'GET',
